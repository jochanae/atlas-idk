...
Atlas exists for builders who often do not know what they do not know. The Mentor Layer is the system that protects them from that gap — not by lecturing, but by capturing confusion the moment it appears and turning it into understanding on the user's own timeline.

The Mentor Layer is not a feature. It is the *teaching half* of the Mentor + Judge system declared in Section III. It runs continuously in Velocity Mode and remains available — though quieter — in Commit Mode.

**The Mentor Layer has three primitives, built in this order:**

### 1. The Parking Lot — *Phase 1*

Any term, suggestion, recommendation, or decision the user does not understand can be one-tapped into a personal queue. No AI is required to capture it. Atlas tags each parked item with:

- **What it is** — term, pattern, decision, tool, or recommendation
- **Where it came from** — what the user was doing or reading when it appeared
- **When it was parked** — so context is preserved

The Parking Lot is the foundation. Everything else in the Mentor Layer feeds from it. The user is never forced to resolve a parked item — it waits until they have bandwidth.

### 2. Glossary-in-Context — *Phase 2*

Any parked item, or any jargon term Atlas itself surfaces, is one tap from a plain-English explanation. The explanation always answers three questions in this order:

1. **What does it mean** — in language the user already understands
2. **Why does it matter for this project** — specific, not generic
3. **Is this decision reversible** — so the user knows whether to slow down or move on

Generated by the lowest-trust model that can do the job (per Section IX) and cached per term per project. Cheap, fast, and always available.

### 3. "What Should I Be Thinking About Now" — *Phase 3*

A single button on the front door. Atlas reads the user's parked items, active project, recent decisions, and unresolved threads — and returns the two or three things that actually matter at this moment. Not a checklist. A focus.

This primitive cannot exist meaningfully until the Parking Lot and the Glossary have been collecting signal. It is built last because it depends on the other two for its data.

---

**The Mentor Layer's law:**
Confusion is signal, not failure. Atlas captures it without judgment, explains it without condescension, and surfaces it back when it is most useful. The user's lack of knowledge is never treated as a problem to be hidden — it is the raw material the system was built to refine.

---

## XII. The Unbreakable Laws
