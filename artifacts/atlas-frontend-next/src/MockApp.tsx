...
import { MockRunProvider } from "@/context/MockRunProvider";
...
/**
 * MockApp — the scripted-lifecycle shell (formerly the default App). Kept as
 * a developer-only regression tool at ?mock=1. Never connects to the backend.
 */
export function MockApp() {
  return (
    <MockRunProvider conversationId="mock-conversation-1">
      <Shell />
    </MockRunProvider>
  );
}
    ...
    <div style={{ display: "grid", gridTemplateRows: "auto 1fr", height: "100%" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
          ...
          <span style={{ color: "var(--muted)", fontSize: 12 }}>Run Contract v1.2 · MOCK</span>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", minHeight: 0 }}>
        <main style={{ display: "flex", flexDirection: "column", minHeight: 0 }}>
          ...
          <section style={{ padding: 20, overflow: "auto", flex: 1 }}>
            {tab === "chat" && <ChatSurface conversationId="mock-conversation-1" useMockActivity />}
        ...
        <aside style={{ borderLeft: "1px solid var(--border)", padding: 20, background: "var(--panel)", overflow: "auto" }}>
          <StoryPanel />
        </aside>
      </div>
    </div>
      ...
      {tabs.map((t) => (
        <button key={t.id} onClick={() => onChange(t.id)}
          style={{
            ...
            color: current === t.id ? "var(--text)" : "var(--muted)", fontSize: 13,
          }}
        >{t.label}</button>
      ))}
    </nav>
...
function StoryPanel() {
  ...
  if (!__startMockRun) return null;
  return (
    <div>
      <h3 style={{ margin: "0 0 4px", fontSize: 14 }}>Lifecycle stories</h3>
      <p style={{ color: "var(--muted)", fontSize: 12, marginTop: 0 }}>Scripted runs (mock).</p>

      <StoryGroup title="BUILD">
        <StoryButton disabled={buildBlocked} onClick={() => __startMockRun("BUILD", "build_success")}>Full success path</StoryButton>
        <StoryButton disabled={buildBlocked} onClick={() => __startMockRun("BUILD", "build_awaiting")}>Stop at confirmation (Gate 1)</StoryButton>
        <StoryButton disabled={buildBlocked} onClick={() => __startMockRun("BUILD", "build_failure")}>Fail mid-execution (partial writes)</StoryButton>
      </StoryGroup>

      <StoryGroup title="CHAT">
        <StoryButton disabled={!!activeTurn} onClick={() => __startMockRun("CHAT")}>Send chat turn</StoryButton>
      </StoryGroup>

      <StoryGroup title="DECIDE">
        <StoryButton disabled={!!activeTurn} onClick={() => __startMockRun("DECIDE")}>Send decide turn</StoryButton>
      </StoryGroup>

      {buildBlocked && (
        <p style={{ color: "var(--warn)", fontSize: 12, marginTop: 16 }}>
          One BUILD is active. Cancel it or wait — per §9.
        </p>
      )}
    </div>
  );
...
function StoryButton({ onClick, disabled, children }: { onClick: () => void; disabled?: boolean; children: React.ReactNode }) {
  return (
