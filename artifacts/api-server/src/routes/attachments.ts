 ...
 *   4. On Nexus/Ask Atlas send with attachmentIds → link chat/nexus message ids
 *   5. GET /message/:messageId returns rows after send completes
 *
 * Frontend contract: artifacts/atlas-frontend/src/lib/attachments/{types,adapter}.ts
 */
...
/** PersistedAttachment — mirrors frontend types.ts exactly. */
...
const RequestUploadBody = z.object({
  ...
  // Optional extensions — frontend adapter currently omits these; set on send.
...
router.post("/attachments/:id/finalize", async (req, res): Promise<void> => {
  ...
  try {
    ...
    // Classification / storage errors → mark failed but keep the row.
    try {
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      // Ownership: attachment.user_id OR message → conversation → user.
      ...
      let allowed = false;
      if (nexusMsg && nexusMsg.userId === userId) {
        allowed = true;
      } else {
      ...
      // Also allow if the caller owns the attachment rows directly
      // (covers rows linked before message ownership tables are consistent).
      const rows = await db
        .select()
        .from(messageAttachmentsTable)
        .where(
      ...
      res.json(rows.map((r: typeof messageAttachmentsTable.$inferSelect) => toPersisted(r)));
    } catch (err) {
      logger.error(
 ...
 *
 * Dual-retain: create a library_items row; keep the conversation attachment
 * row pointing at the same storage object (original reference never breaks).
 */
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      // Idempotent: if already promoted, return current row.
      ...
      const [libItem] = await db
        ...
        .values({
          ...
          originSource:
            row.surface === "nexus" ? "workspace" : "ask-atlas",
 ...
 * 302 to a short-lived signed GET URL.
 */
router.get("/attachments/:id/download", async (req, res): Promise<void> => {
  ...
  try {
    // Prefer redirect when storage signing works; stream as fallback.
    const { bucketName, objectName } = resolveStoredObject({
      storageBucket: row.storageBucket,
      storagePath: row.storagePath,
    });
