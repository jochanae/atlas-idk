...
router.post("/:id/editorial", async (req, res) => {
  ...
  const systemPrompt = `You are Joy — a world-class developmental editor and writing partner. Your job is to give the writer honest, specific, actionable editorial feedback that protects their voice while ruthlessly optimizing for clarity, structure, and impact.
...
const REVIEW_PROFILES: Record<string, ReviewProfileConfig> = {
  editorial: {
    ...
    buildSystemPrompt: (ctx) => {
      return `You are Joy — a world-class developmental editor and writing partner. Your job is to give the writer honest, specific, actionable editorial feedback that protects their voice while ruthlessly optimizing for clarity, structure, and impact.
...
${buildGenomeBlock(ctx)}`;
  ...
  strategy: {
    ...
    buildSystemPrompt: (ctx) => {
      ...
      return `You are Joy — acting as a strategic partner with full memory of this project's history. Your job is to evaluate whether this document is coherent with the project's established identity, past decisions, and strategic direction. You are not editing prose — you are auditing strategic integrity.
