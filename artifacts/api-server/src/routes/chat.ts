...
import { ATLAS_IDENTITY, ATLAS_COMMUNICATION_STYLE, ATLAS_DESIGN_INTELLIGENCE } from "../lib/atlasIdentity";
...
You are Joy (internally: Atlas). Just be it.
...
async function runDeepDive(topic: string, systemPrompt: string): Promise<ModelCallResult> {
  ...
  const prompt = `You are Joy (internally: Atlas) performing a Deep Dive research analysis. The user wants comprehensive technical insight on:
...
Do not suggest style improvements or preferences. Only flag genuine problems.`,
  ...
  if (structuredPlanArtifact) {
    _chatRunIntermediateSteps.push({
      verb: "PLAN_RECORDED",
      target: structuredPlanArtifact.title ?? null,
      detail: `${structuredPlanArtifact.steps?.length ?? 0} steps · confidence ${structuredPlanArtifact.confidence ?? "unknown"}`,
      content: null,
    });
  }
...
router.post("/specify", async (req, res) => {
  ...
  const specSystemPrompt = `You are Joy (internally: Atlas) — the strategic intelligence inside Axiom. Convert raw human intent into a precise 10-section change specification that acts as a boundary document before any AI builder touches the code.
...
router.post("/quick-prompt", async (req, res) => {
  ...
  const platformPrompts: Record<string, string> = {
    Cursor: `You are Joy (internally: Atlas) — the strategic intelligence inside Axiom, built for a solo founder who builds production SaaS entirely on her phone using Cursor Agent.
...
- Never reference "Atlas" or "Axiom" in the output — Cursor doesn't know what that is.`,
    ...
    Replit: `You are Joy (internally: Atlas) — the strategic intelligence inside Axiom. Generate a precise, ready-to-paste Replit Agent prompt.
...
- Never reference "Atlas" or "Axiom" in the output.`,
    ...
    Lovable: `You are Joy (internally: Atlas) — the strategic intelligence inside Axiom. Generate a precise, ready-to-paste Lovable prompt.
...
- Never reference "Atlas" or "Axiom" in the output.`,
    ...
    Bolt: `You are Joy (internally: Atlas) — the strategic intelligence inside Axiom. Generate a precise, ready-to-paste Bolt prompt.
...
- Never reference "Atlas" or "Axiom" in the output.`,
    ...
    v0: `You are Joy (internally: Atlas) — the strategic intelligence inside Axiom. Generate a precise, ready-to-paste v0 prompt.
...
- Never reference "Atlas" or "Axiom" in the output.`,
    ...
    Claude: `You are Joy (internally: Atlas) — the strategic intelligence inside Axiom. Generate a precise, ready-to-paste Claude prompt.
