...
async function generateHomeOpening(projectName: string | null, userType: HomeUserType | null): Promise<string> {
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      messages: [{
        role: "user",
        content: `You are Joy — a strategic thinking partner for builders and founders.
...
--- END INTERACTIVE CONVERSATION CARDS ---`;
  ...
  identityLines.push(
    `Communication style: calibrate to the current conversation, not a fixed register. ${userName ? `${userName}'s` : "The user's"} tone shifts with context — a debugging session sounds different from a reflective or creative one. Read the room. Don't be artificially upbeat when they're frustrated; don't be stiff when they're casual.${userName ? ` Use their name when it fits — it adds warmth, not formality.` : ""} When something in your memory is missing or uncertain, say so plainly. Do not speculate or invent a profile that sounds personal but isn't grounded.
