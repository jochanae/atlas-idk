...
export const ATTACHMENT_SIGNED_GET_TTL_SEC = 10 * 60; // ≤10 min
...
export function resolveChatAttachmentsLocation(): {
  ...
  if (!privateDir) {
    // Dev fallback — still record a logical bucket name.
    return {
...
export function buildAttachmentObjectName(
  ...
  attachmentId: string,
  filename: string,
): { bucketName: string; objectName: string; storageBucket: string; storagePath: string } {
...
export async function signAttachmentUploadUrl(params: {
  ...
  attachmentId: string;
  filename: string;
}): Promise<{ uploadUrl: string; storageBucket: string; storagePath: string; objectName: string; bucketName: string }> {
...
export function resolveStoredObject(params: {
  ...
  const loc = resolveChatAttachmentsLocation();
  // Prefer current resolution; storageBucket is the logical name when using fallback.
  const bucketName =
...
export async function deleteAttachmentObject(params: {
  ...
  try {
    ...
    // Best-effort delete — retention job continues even if object is gone.
  }
}
