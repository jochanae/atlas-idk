...
export function classifyTerminalCommand(
  ...
  if (matchesAny(normalized, [
    ...
    /^(?:ls|pwd|cat|head|tail|grep|find|wc|sort|uniq|cut|awk|sed|tr)\b/i,
    ...
    /^node\s+--version$/i,
    /^npm\s+--version$/i,
    // Safe system introspection commands
    /^whoami$/i,
    /^id(?:\s|$)/i,
    /^uname(?:\s|$)/i,
    /^hostname(?:\s|$)/i,
    ...
    /^file\b/i,
    /^stat\b/i,
    /^pnpm\s+(?:run\s+)?(?:typecheck|build|test|lint)\b/i,
    /^pnpm\s+--filter\s+\S+\s+run\s+(?:typecheck|build|test|lint)\b/i,
  ])) {
