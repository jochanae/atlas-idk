 ...
 *
 * Start once after the server is listening:
...
export async function runAttachmentRetentionPass(): Promise<{
  ...
  // 1. active → expiring when within 7 days of expiry (but not yet past)
  ...
  // 2. active/expiring → expired when past expires_at (library rows exempt)
  ...
  // 3. Delete storage objects for just-expired rows (keep DB row)
