...
function RightPanel({
  ...
  return (
    ...
    >
      ...
      {tab === "insights" && (
        <InsightsPanel projectId={projectId} onOpenFlow={() => setTab("map")} />
      )}
...
export default function Workspace() {
  ...
  useEffect(() => {
    fetchWorkspaceAtlasConversations();
  }, [fetchWorkspaceAtlasConversations]);
  useEffect(() => {
    if (!showDrawer) return;
    fetchWorkspaceAtlasConversations();
  }, [showDrawer, fetchWorkspaceAtlasConversations]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener("axiom:atlas-download-chat", onDownload);
    };
  }, [isAtlasScope, handleNewSession, downloadConversation]);
  ...
  useEffect(() => {
    ...
    const hasCarryover = convKey ? (() => { try { return sessionStorage.getItem(convKey) === "1"; } catch { return false; } })() : false;
          ...
          <UnifiedConversationSurface
            ...
            composerProps={(isMobile && mobileTab !== "chat") ? null : {
              ...
              onComposerMenuAction: (action) => {
                ...
                if (action === "park") { setParkPrefill(""); setShowParkSheet(true); return; }
