...
export function GitHubPushModal({
  ...
  const [commitMsg, setCommitMsg] = useState(
    fileEdits.length === 1
      ? `Joy: update ${fileEdits[0]?.path.split("/").pop() ?? "file"}`
      : `Joy: update ${fileEdits.length} files`
  );
      ...
      for (let i = 0; i < fileEdits.length; i++) {
        ...
        const r = await fetch("/api/github/commit", {
          ...
          body: JSON.stringify({
            ...
            content: orig, message: `Joy: rollback ${fileEdits[i].path.split("/").pop()}`,
          }),
        });
              ...
              <div style={{
                ...
                {typecheckResult.errors.length > 6 && (
                  <div style={{ fontFamily: "var(--app-font-mono)", fontSize: 10, color: "var(--atlas-muted)", marginTop: 3 }}>
                    +{typecheckResult.errors.length - 6} more — share with Joy to fix
                  </div>
                )}
              </div>
            )}
