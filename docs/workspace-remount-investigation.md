...
## 6. Task #172 regression pass (fresh, live-code re-check)

Re-verified each item from §4 against current `workspace.tsx`/`home.tsx`/`projects.tsx`/`ViewChangesPanel.tsx` code (not live browser testing this round — static re-check via grep/read):

- **Tab-focus flicker fix** (§5, `refetchOnWindowFocus: false` in `home.tsx`/`projects.tsx`) — **still intact, no regression**.
- **Header stale-snapshot fix** (`workspace.tsx` ~line 7654-7669, resets `currentSnapshot`/`handoverOpen` on project-id change) — **still intact, no regression**.
- **ViewChangesPanel duplicate-request fix** (`workspace.tsx` ~line 4604-4625, polls `/api/projects/:id/runs` once and writes into the shared `["project-runs", id]` React Query cache key) — **still intact, no regression**.
- **Vite HMR full-reload risk** — not independently re-triggered this pass; no new HMR-config changes found since the prior investigation, so treated as unchanged/no regression.

### §3 correction — the actually-rendered chat transport is `/api/nexus/chat`, not `/api/chat`

§3 above concluded `/api/chat` (via `useChatStream.ts`) was the live path and `nexus.ts` was dormant. **That conclusion is now stale for the current codebase.** `workspace.tsx` sets `useNexusWorkspaceChat = true` (hardcoded) and the `ChatStream`/composer prop-spreads at ~line 8801 and ~8934 apply `...(useNexusWorkspaceChat ? { messages: nexusBridge.messages, ... } : {})` **last**, so `nexusBridge` (backed by `useNexusChatStream` → `POST /api/nexus/chat`) always wins over `useChatStream`'s return value in what actually renders and sends.

Consequence found and fixed in this task: `useNexusWorkspaceBridge.ts`'s `toChatMessage()` mapper did not carry `generatedArtifacts` (the file-backed-deliverable field added for task #171's `ArtifactCreatedCard`) from `NexusMessage` to `ChatMessage`, and `useNexusChatStream.ts`'s own `NexusMessage` type/done-handler never captured `generatedArtifacts` from the `/api/nexus/chat` SSE `done` event at all — even though `nexus.ts`'s backend handler already includes `generatedArtifacts` in that payload (it shares `generate_deliverable` via the common `SHARED_WORKSPACE_TOOL_NAMES` tool registry). Net effect: task #171's deliverable receipt card silently never rendered on the actually-live chat path. Fixed by adding the `generatedArtifacts` field to `NexusMessage`, capturing it in the done-event handler, and mapping it through in `toChatMessage()`.

**Not fixed (flagged as follow-up, out of scope for a targeted fix):** `workspace.tsx` still unconditionally instantiates *both* `useNexusWorkspaceBridge` and `useChatStream` on every render, even though only the former is ever read into the UI. `useChatStream` independently fires its own `useListMessages(sessionId)` fetch and holds other live side effects, none of which are gated behind `useNexusWorkspaceChat`. This is wasted duplicate network/state work at minimum, and a latent divergence risk (e.g. `sessionId` bookkeeping happening in a hook whose output is discarded) at worst. De-risking this needs either a feature flag to fully disable `useChatStream` when Nexus is active, or finishing the migration referenced in memory (`nexus-workspace-spine.md`, "Phase 2 = migrate 12 useChatStream side effects") — both are larger changes than this task's targeted-fix scope, so this is filed as a follow-up rather than patched here.
