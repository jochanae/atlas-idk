...
**Gotcha:** not every renderer generates new content from an LLM. The "bundle" (Ship Package) renderer instead reads other already-generated file-backed artifacts (via `getFileBackedArtifact` + object storage download) and zips them — it needed its own generate route (`/bundles/generate`) rather than reusing `/deliverables/:type/generate`, since that route always requires conversation `context` which a pure-repackaging renderer has no use for. Any future renderer whose input is "other artifacts" rather than "conversation context" should follow this same pattern: own route, same underlying `generateArtifact` engine call.

**Gotcha:** after adding/registering a new renderer or route in the dev workflow, changes are NOT live until the workflow restarts — the dev script only builds once at process start. A route added while the server is running from before the edit will silently mismatch (returning a stale/wrong handler's response, e.g. a 403 from an unrelated middleware) instead of a clean 404. Always restart the workflow before curl-testing a newly added route.
