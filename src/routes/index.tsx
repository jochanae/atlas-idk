...
function WorkspacePage() {
  ...
  // ── Rollback & History ──
  const handleRollback = useCallback(async (targetMessage: ChatMessage) => {
    if (!user || !activeProjectId || !session) return;
    // Snapshot: all messages up to and including this one
  ...
  const confirmRollback = useCallback(async () => {
    ...
    // Log to ledger as System Reversion
    try {
    ...
    // Gold glow pulse on the rolled-back message
  ...
  const mainShell = (
    <div className="atlas-shell-host min-h-screen h-full bg-background text-foreground overflow-hidden flex flex-col">
