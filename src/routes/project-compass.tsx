...
export const Route = createFileRoute("/project-compass")({
  component: ProjectCompass,
  head: () => ({
    meta: [
      { title: "Atlas — Project Compass" },
      {
        name: "description",
        content:
          "Decision health across all projects. Committed, parked, and overridden at a glance.",
      },
    ],
  }),
});
...
function ProjectCompass() {
  ...
  const [entriesByProject, setEntriesByProject] = useState<Record<string, Entry[]>>({});
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [{ data: projs }, { data: ents }] = await Promise.all([
        ...
        (supabase as unknown as { from: (t: "entries") => {
          select: (c: string) => {
            in: (col: string, vals: unknown[]) => Promise<{ data: Entry[] | null }>;
          };
        }}).from("entries").select("*").in("status", ["committed", "parked"]),
      ]);
      if (cancelled) return;
...
function ProjectHealthCard({
  ...
  const { committed, parked, violations, total, healthRate, topModes } = useMemo(() => {
    const committed = entries.filter((e) => e.status === "committed").length;
    const parked = entries.filter((e) => e.status === "parked").length;
    const violations = entries.filter((e) => e.is_violation).length;
    ...
    return { committed, parked, violations, total, healthRate, topModes };
  }, [entries]);
